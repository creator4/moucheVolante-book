...
function drawDebugElementLabel(label, x, y, dx = 22, dy = -22) {
  drawingContext.save();
  drawingContext.font = '700 14px system-ui, -apple-system, sans-serif';
  drawingContext.fillStyle = '#22c55e';
  drawingContext.shadowColor = 'rgba(0,0,0,0.65)';
  drawingContext.shadowBlur = 5;
  drawingContext.textBaseline = 'middle';
  drawingContext.fillText(label, x + dx, y + dy);
  drawingContext.restore();
}

function drawThinObjectOutline(x, y, r, aspect = 1) {
  drawingContext.save();
  noFill();
  stroke(255, 70, 190, 210);
  strokeWeight(1);
  ellipse(x, y, r * 2.25 * aspect, r * 2.0);
  drawingContext.restore();
}
...
function draw() {
  ...
  [
    { key: 'rundtest-extra-1', x: 0.27, y: 0.43, r: 0.40, amp: 12, keepSoft: true },
    ...
    drawingContext.save();
    drawingContext.globalAlpha = rt.keepSoft ? 0.70 * ELEMENT_ALPHA : 0.94 * ELEMENT_ALPHA * NON_HF_ELEMENT_ALPHA;
  ...
  nucleusDrops.forEach(d => {
    ...
    const compareLabel = d._compareLabel || d._label || (d.semiTiedTo && (d.semiTiedTo._compareLabel || d.semiTiedTo._label));
    const visibilityFactor = elementVisibilityFactor(compareLabel);
    ...
    if (a !== 1 || s !== 1) {
      ...
      drawingContext.globalAlpha *= a * ELEMENT_ALPHA * visibilityFactor;
      if (s !== 1) {
      ...
      drawingContext.save();
      if (faded) drawingContext.globalAlpha *= 0.72;
      drawingContext.globalAlpha *= ELEMENT_ALPHA * visibilityFactor;
      d.display();
      drawingContext.restore();
    }
  });
  nucleusDrops.forEach(d => {
    ...
    if (compareVariantVisible(compareLabel, 'hf')) {
      ...
      drawingContext.save();
      const hfVisibilityFactor = compareLabel === 'Nucleus 1' ? 2.08 : visibilityFactor;
      drawingContext.globalAlpha *= ELEMENT_ALPHA * hfVisibilityFactor;
    ...
    if (compareVariantVisible(compareLabel, 'slime')) {
      if (compareLabel === 'Dreieck') {
        ...
        drawingContext.save();
        drawingContext.globalAlpha *= 0.44 * ELEMENT_ALPHA * visibilityFactor;
        drawingContext.save();
        drawingContext.globalAlpha *= ELEMENT_ALPHA * visibilityFactor;
